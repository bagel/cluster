...
def online(environ):
    ...
    ip = urlparse.parse_qs(environ['QUERY_STRING'])['ip'][0]
    admin = load = desp = port = 0
    dp_data = json.loads(_request(url=dpurl+ip))
    if not dp_data['list']:
        return (ctype, 'null')
    ...
    if dp_data['desp']:
        desp = 1
    if json.loads(_request(url=wburl+dp_data['ip_in'])) or \
       json.loads(_request(url=wburl+dp_data['ip_ex'])):
        load = 1
    try:
        if re.match('OK', _request(url='http://%s' % ip)):
            port = 1
    except:
        pass
...
def config(environ):
    ...
    conf.read(os.path.join(environ['DOCUMENT_ROOT'], 'wsgi/tools/ip.cfg'))
    sec = cgi.parse_qs(environ['QUERY_STRING'])['cfg'][0]
    ip = cgi.parse_qs(environ['QUERY_STRING'])['ip'][0]
    print ip, sec
...
def publish(environ):
    ctype = 'text/plain; charset=utf-8'
    cfg = cgi.parse_qs(environ['QUERY_STRING'])['cfg'][0]
    print cfg
...
def logset(environ):
    ...
    print "work"
    p = urlparse.parse_qs(urllib2.unquote(environ['QUERY_STRING']))
    print p
    k = p['file'][0]
...
def logget(environ):
    ...
    file = urlparse.parse_qs(environ['QUERY_STRING'])['file'][0]
